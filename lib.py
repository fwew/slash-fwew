...
def get_language(inter):
    channel_languages = {
        ...
        1478547500964253716: "pt",  # LN/Português/pängkxo-hu-bot
...
def format_pages_dictionary(
    words: str, languageCode: str, showIPA: bool = False, reef: bool = False
):
...
def format_pages_dictionary_helper(
    words: str,
    languageCode: str,
    showIPA: bool = False,
    row: int = 0,
        ...
        while j < len(words):
            ...
            if showIPA:
                ...
                if "ʊ" in ipa:
                    results += f"or [{ipa}] "
            results += (
                f"({breakdown}) *{word['PartOfSpeech']}* {word[languageCode.upper()]}\n"
            )
...
def format_pages_helper(
    words: str, languageCode: str, showIPA: bool = False, row: int = 0
) -> str:
    results = ""
    if len(words) == 0:
        if row == 0:
    ...
    if not hasWords:
        embeds = [
            disnake.Embed(
                ...
                description="No Na'vi words found for:\n" + words,
            )
        ]
    ...
    if not hasWords:
        embeds = [
            disnake.Embed(
                color=Colour.orange(),
                title="No words found",
                description="No natural language words found for:\n" + words,
            )
        ]
...
def get_search(
    languageCode: str, words: str, showIPA: bool = False, reef: bool = False
):
    embeds = []
    ...
    if not hasWords:
        embeds = [
            disnake.Embed(
                color=Colour.orange(),
                title="No words found",
                description="No multiple IPA words found:\n",
            )
        ]
    ...
    if type(results) == str:
        embeds.append(
            disnake.Embed(
                color=Colour.orange(),
                title="Random failed",
                description="You should not be seeing this on Discord",
            )
        )
    else:
        for a in results:
...
def get_random_filter(
    languageCode: str, n: int, args: str, showIPA: bool, check_digraphs: str
) -> str:
...
def get_name(ending: str, n: int, dialect: str, s1: int, s2: int, s3: int) -> str:
    ...
    return json.loads(
        requests.get(
            f"{api_url}/name/full/d/{ending}/{n}/{s1}/{s2}/{s3}/{dialect}"
        ).text
    )
...
def get_name_alu(n: int, dialect: str, s: int, noun_mode: str, adj_mode: str) -> str:
    return json.loads(
        requests.get(
            f"{api_url}/name/alu/{n}/{s}/{noun_mode}/{adj_mode}/{dialect}"
        ).text
    )
...
def get_phonemes(lang: str) -> str:
    all_frequencies = json.loads(requests.get(f"{api_url}/phonemedistros/{lang}").text)
